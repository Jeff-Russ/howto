...
$( document ).ready(function() {
   ...
   // add click event handler to all <a href="#ANYTHING" >
   $('a[href^="#"]').on('click', function (e) {   // e == object that raised the event
      e.preventDefault();           // bypass clicked <a>'s native bahavior 
      $(document).off("scroll");    // remove event handler on scroll

      $('a').each( function () { $(this).removeClass('active'); } ); //reset all
      
      $(this).addClass('active');   // add our own active status to clicked <a> 
   ...
   // listen to user and animated scroll:
   function onScroll(event) {
      ...
      $('nav a').each(function () { 
         ...
         if (refElement.position().top + -200 // OFFSET 50 for navbar height + more 
            <= scrollPosition && refElement.position().top + 
            refElement.height() > scrollPosition) { // if it is near top of page:
            $('nav ul li a').removeClass("active"); // then reset all to not active
            currentLink.addClass("active");         //  and add active to match
         }else{currentLink.removeClass("active"); } // if it is not a match, reset
      });
   }// END scroll tracker
